...
//$date = date('Y-m-d H:i:s', strtotime($input['curr_year']."-".$input['last_month']."-1"));
$date = date('Y-m-d H:i:s', strtotime("2020-1-1")); // for testing
$usernames[] = array();
$userids[] = array();
...
if(isset($userid) and isset($usertype)){
  ...
  //get usernames
  $sql = $conn->query("SELECT DISTINCT username,user.userid as user_id FROM user INNER JOIN record on user.userid=record.userid WHERE activity_timestamp>'$date'");
  if($sql){
  ...
  $sql = $conn->query("SELECT COUNT(*) as vscore FROM record WHERE activity_type='IN_VEHICLE' AND activity_timestamp>'$date' GROUP BY userid");
  if($sql){
    while($row = $sql->fetch_assoc()) {
  ...
  $sql = $conn->query("SELECT COUNT(*) as pscore FROM record WHERE (activity_type='ON_BICYCLE' OR activity_type='ON_FOOT' OR activity_type='RUNNING' OR activity_type='WALKING') AND activity_timestamp>'$date' GROUP BY userid");
  if($sql){
    while($row = $sql->fetch_assoc()) {
  ...
  for ($i=0; $i <sizeof($vehicle_scores)-1; $i++) {
    if(intval($vehicle_scores[$i+1])==0){
      $scores[$i] = 100;
    }
    else{
      $scores[$i] = intval(intval($physical_scores[$i+1]) * 100 / intval($vehicle_scores[$i+1]));
    }
  }
  #find user rank
